...
def parse_args():
    ...
    tn.add_argument("--editor",
                    choices=['A', 'B', 'C', 'D', 'E'],
                    default='C',
                    help="Which editor images to use for training")

    tn.add_argument("--print-every-x-batches",
                    type=int,
                    default=hp.print_every_x_batches,
                    help="After how many batches you want to print")

    tn.add_argument("--save-every-x-batches",
                    type=int,
                    default=hp.save_every_x_batches,
    ...
    ts.add_argument(
        ...
        default=True,
        help=
        "True if you want to display the test output, false to save to file.")
    ...
    pf.add_argument("--untouched-dir",
                    type=valid_dir,
                    default=os.getcwd() + "/data/train/untouched",
                    help="Directory of untouched images for training")

    pf.add_argument("--edited-dir",
                    type=valid_dir,
                    default=os.getcwd() + "/data/train/edited",
                    help="Directory of expert edited images for training")

    pf.add_argument("--editor",
                    choices=['A', 'B', 'C', 'D', 'E'],
                    default='C',
                    help="Which editor images to use for training")
		...
		for b, batch in enumerate(dataset.data):
			...
			for _ in range(hp.gen_update_freq):
				with tf.GradientTape() as gen_tape:
                    ...
                    y_model = editor.PhotoEditor.edit(x_model.numpy(),
                                                      act_scaled)
            ...
            if b % ARGS.print_every_x_batches == 0:
                print(
                    "Epoch: {} Batch: {} Generator Loss: {} Discriminator Loss: {}"
                    .format(e, b, gen_loss, disc_loss))
...
def performance(dataset, generator):
    ...
    for batch in dataset.data:
        ...
        ssim_val += ssim(y_model,
                         batch[:, 1].numpy(),
                         data_range=1,
                         multichannel=True)
        psnr_rand += editor.PSNR(rand_edits, batch[:, 1])
        ssim_rand += ssim(rand_edits,
                          batch[:, 1].numpy(),
                          data_range=1,
                          multichannel=True)
        batch_count += 1
    print("MODEL -> PSNR: {} SSIM: {} RANDOM -> PSNR: {} SSIM: {}".format(
        psnr / batch_count, ssim_val / batch_count, psnr_rand / batch_count,
...
def edit_original(big_image, generator):
    ...
    '''
    resized = resize(big_image, (hp.img_size, hp.img_size))

    prob, _ = generator([resized])
    act_scaled, _ = generator.convert_prob_act(prob.numpy())

    orig_edit = editor.PhotoEditor.edit(big_image.numpy(), act_scaled)
    ...
    try:
        with tf.device("/device:" + ARGS.device):
            if ARGS.command == "train":
                # train here!
                dataset = Datasets(ARGS.untouched_dir, ARGS.edited_dir,
                                   "train", ARGS.editor)
